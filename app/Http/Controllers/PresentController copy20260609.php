...
// dd($request->work_time[0]);
    ...
    public function testAjax(Request $request) {
        ...
        $resume_nickname = $request->input('resume_nickname');
        $resume_picme = $request->input('resume_picme');
        $resume_sex = $request->input('resume_sex');
        $resume_age = $request->input('resume_age');
        $resume_marry = $request->input('resume_marry');
        $resume_education = $request->input('resume_education');
        $resume_cellphone = $request->input('resume_cellphone');
        $resume_email = $request->input('resume_email');
        $resume_introduction = $request->input('resume_introduction');
        $resume_summary = $request->input('resume_summary');
        $resume_display = $request->input('resume_display');
        $resume_experiences = json_encode($request->input('resume_experiences'), JSON_UNESCAPED_UNICODE);
        $resume_skills = json_encode($request->input('resume_skills'), JSON_UNESCAPED_UNICODE);
        
        // 创建验证规则：name字段为必填项
        $validator = Validator::make($request->all(),
             ['resume_name' => 'required', 'string', 'regex:/^[\x{4e00}-\x{9fa5}a-zA-Z\s]{2,30}$/u'],
             ['resume_nickname' => 'required', 'string', 'regex:/^[\x{4e00}-\x{9fa5}a-zA-Z\s]{2,30}$/u'],
             ['resume_education' => 'required', 'string', 'regex:/^[\x{4e00}-\x{9fa5}a-zA-Z\s]{2,30}$/u'],
             ['resume_cellphone' => 'required', 'string', 'regex:/^[\x{4e00}-\x{9fa5}a-zA-Z\s]{2,30}$/u'],
             ['resume_email' => 'required', 'string', 'regex:/^[^\s@]+@[^\s@]+\.[^\s@]+$/'],
             ['resume_introduction' => 'required', 'string', 'regex:/^[\x{4e00}-\x{9fa5}a-zA-Z\s]{2,30}$/u'],
             ['resume_summary' => 'required', 'string', 'regex:/^[\x{4e00}-\x{9fa5}a-zA-Z\s]{2,30}$/u'],
             // 多筆動態陣列驗證
             ['resume_experiences' => 'nullable', 'array'],
             ['resume_experiences*' => 'required', 'string', 'regex:/^[\x{4e00}-\x{9fa5}a-zA-Z\s]{2,30}$/u'], 
             ['resume_skills' => 'nullable', 'array'],
             ['resume_skills*' => 'required', 'string', 'regex:/^[\x{4e00}-\x{9fa5}a-zA-Z\s]{2,30}$/u'],
             // 檔案驗證
             ['resume_picme' => 'nullable', 'image', 'mimes:jpeg,png,jpg', 'max:2048']);
        if ($validator->fails()){
            // 验证失败时返回JSON格式的错误信息
            $errors = $validator->errors();
            return response()->json(['errors' => $errors], 422);
        } else{
            if ($request->hasFile('resume_picme') && isset($resume_picme)) {
                // 原始檔名
                $fileName = $_FILES['resume_picme']['name'];
                // 取出原始檔名的檔案名稱
                $name = pathinfo($fileName, PATHINFO_FILENAME);
                ...
                $image = Image::make($pic)->save($file_path);
                // 設定圖片檔案相對路徑
                $resume_picme = $file_relative_path;
            }else{
                return response()->json(['errors' => $errors], 422);
            }
            present::create([
            ...
            // 验证成功时返回JSON格式的欢迎消息
            return response()->json(['success' => "welcome ". $resume_experiences,'success2' => "welcome ". $resume_name]);
        }
    ...
    //     $resume_name = $request->input('resume_name');
    //     // 後端再判斷一次 (Validation)
    //     $validator = Validator::make($request->all(), ['resume_name' => 'required']);
    ...
    //         'resume_email' => $request->resume_email,
    //         'resume_introduction' => $request->resume_introduction,
    //         'resume_experience' => $request->resume_experiences, // 透過 Model cast 自動轉 JSON
    //         'resume_skill' => $request->resume_skills,
    //         'resume_summary' => $request->resume_summary,
    //         'resume_display' => $request->resume_display,
    //     ]);
