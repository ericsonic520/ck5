      ...
      <div class="card-body">
        <table class="table table-bordered">
          <tbody class="text-center">
              ...
              <tr>
                  <td>{{ $Post->post_id }}</td>
                  <td>{{ $Post->sort_name }}</td>
                  <td><a href="news/{{ $Post->post_id }}/itm" style="color: black;text-decoration: none;" title="{{ $Post->post_title }}">{{ $Post->post_title }}</a></td>
                  <td>
                      <form action="/news/{{ $Post->post_id }}/chgnewsdis" enctype="multipart/form-data">
                        @if($Post->post_display=='1')
                          <button method="submit" class="btn btn-success" title="顯示"><i class="far fa-eye"></i></button>
                        @endif
                        @if($Post->post_display=='0')
                          <button method="submit" class="btn btn-warning" title="隱藏"><i class="fas fa-solid fa-eye-slash"></i></button>
                        @endif
                        <input type="hidden" name="post_display" value="{{$Post->post_display}}">
                      </form> 
                  </td>
                  <td>
                  ...
                  @guest
                    @else 
                      @if(Auth::user()->type=='A')
                      <a href="/news/{{ $Post->post_id }}/itm" title="頁面連結">
                      <button class="btn btn-info"><i class="fas fa-solid fa-list"></i></button>
                      </a>
                        <a href="/news/{{ $Post->post_id }}/edit" title="更新內容">
