        ...
        <div class="card-body">
            <form id="ajaxForm" enctype="multipart/form-data">
                <div class="container">
                    ...
                    <div class="form-group col-md-3">
                        ...
                        <textarea class="form-control" type="text" name="resume_introduction" id="resume_introduction" placeholder="請輸入簡介"></textarea>
                        <div class="invalid_resume_introduction"></div>
                    </div>
                    ...
                    <div class="form-group col-md-3">
                        ...
                        <input id="resume_picme" type="file" name="resume_picme" placeholder="resume_picme" accept="image/*">
                        <div class="invalid_resume_picme"></div>
                    </div>
            ...
            <script>
                ...
                $('#ajaxForm').submit(function(e){
                    ...
                    let resume_introduction = $("#resume_introduction").val().trim();
                    let resume_picme = $('#resume_picme')[0].files[0];
                    let resume_summary = $("#resume_summary").val().trim();
                    let resume_display = $("#resume_display").val().trim();
                    ...
                    let resume_age_reg = RegExp(/^(?:[1-9]|[1-9][0-9]|1[01][0-9]|120)$/);
                    let resume_education_reg = RegExp(/^[A-Za-z\s\u4e00-\u9fa5]+$/);
                    let resume_introduction_reg = RegExp(/^[\u4e00-\u9fa5a-zA-Z0-9\s,，.。]*$/);
                    let resume_summary_reg = RegExp(/^[\u4e00-\u9fa5a-zA-Z0-9\s,，.。]*$/);
                    ...
                    if(resume_name){
                        if( !resume_name_reg.test(resume_name) ){
                            $("#resume_name").focus();
                            invalid_name_feedback = "【履歷名稱】格式不正確（請輸入 2~5 字中文）";
                            isFormValid = false;
                        }else{
                            $("#resume_name").focus();
                            invalid_name_feedback = "";
                            isFormValid = true;
                        }
                    }else{
                    ...
                    if(resume_nickname){
                        if( !resume_nickname_reg.test(resume_nickname) ){
                            ...
                            invalid_nickname_feedback = "【姓名】格式不正確（請輸入 2~5 字中文）";
                            isFormValid = false;
                        }else{
                            $("#resume_nickname").focus();
                            invalid_nickname_feedback = "";
                            isFormValid = true;
                        }
                    }else{
                        invalid_nickname_feedback = "【姓名】不可為空白";
                        isFormValid = false;
                    }
                    ...
                    if(resume_age){
                        if( !resume_age_reg.test(resume_age) ){
                            $("#resume_age").focus();
                            invalid_age_feedback = "【年齡】格式不正確（請輸入 介於 1~100 的數字 ）";
                            isFormValid = false;
                        }else{
                            $("#resume_age").focus();
                            invalid_age_feedback = "";
                            isFormValid = true;
                        }
                    }else{
                    ...
                    if(resume_education){
                        if( !resume_education_reg.test(resume_education) ){
                            ...
                            invalid_education_feedback = "【畢業學校】格式不正確（請輸入 中文或英文 ）";
                            isFormValid = false;
                        }else{
                            $("#resume_education").focus();
                            invalid_education_feedback = "";
                            isFormValid = true;
                        }
                    }else{
                    ...
                    if(resume_cellphone){
                        if( !resume_cellphone_reg.test(resume_cellphone) ){
                            ...
                            invalid_cellphone_feedback = "【手機】格式不正確（請輸入 10 碼台灣手機號碼）";
                            isFormValid = false;
                        }else{
                            $("#resume_cellphone").focus();
                            invalid_cellphone_feedback = "";
                            isFormValid = true;
                        }
                    }else{
                    ...
                    if(resume_email){
                        if( !resume_email_reg.test(resume_email) ){
                            $("#resume_email").focus();
                            invalid_email_feedback = "【信箱】格式不正確";
                            isFormValid = false;
                        }else{
                            $("#resume_email").focus();
                    ...
                    if(resume_introduction){
                        if( !resume_introduction_reg.test(resume_introduction) ){
                            $("#resume_introduction").focus();
                            invalid_introduction_feedback = "【簡介】格式不正確";
                            isFormValid = false;
                        }else{
                            $("#resume_introduction").focus();
                            invalid_introduction_feedback = "";
                            isFormValid = true;
                        }
                    }else{
                        invalid_introduction_feedback = "【簡介】不可為空白";
                        isFormValid = false;
                    }

                    // 【求職簡介】正則表達式判斷
                    if(resume_summary){
                        if( !resume_summary_reg.test(resume_summary) ){
                            $("#resume_summary").focus();
                            invalid_summary_feedback = "【求職簡介】格式不正確";
                            isFormValid = false;
                        }else{
                            $("#resume_summary").focus();
                    ...
                    // 個人頭像驗證
                    var resume_picme = $('resume_picme')[0].files[0];
                    if (!resume_picme) { alert('請上傳個人頭像圖片'); return; }


    // 確保使用者有選取檔案（防止點擊取消）
    if (resume_picme) {
        // 印出圖片的類型 (MIME Type)
        console.log("檔案類型為：", resume_picme.type); 
        
        // 範例：你可以用它來做簡單的判斷
        if (resume_picme.type === 'image/jpeg' || resume_picme.type === 'image/png') {
            alert('這是一張 JPG 或 PNG 圖片！');
        } else {
            alert('不支援此檔案格式，請上傳圖片。');
        }
    }
                    
                    // 圖片檢查（非必填，但有上傳就檢查格式與大小）
                    if (resume_picme) {
                        const allowedTypes = ['image/jpeg', 'image/png', 'image/jpg'];
                        if (!allowedTypes.includes(resume_picme.type)) {
                            invalid_picme_feedback = '圖片格式僅限 JPG, JPEG, PNG。';
                            isFormValid = false;
                        } else if (resume_picme.size > 2 * 1024 * 1024) { // 2MB
                            invalid_picme_feedback = '檔案大小不能超過 2MB。';
                            isFormValid = false;
                        }
                        $("#resume_picme").focus();
                        invalid_picme_feedback = "y";
                        isFormValid = true;
                    }
                    ...
                    $(".invalid_resume_introduction").html(invalid_introduction_feedback).css({"color":"red","font-weight":"bold"});
                    $(".invalid_resume_summary").html(invalid_summary_feedback).css({"color":"red","font-weight":"bold"});
                    $(".invalid_resume_picme").html(invalid_picme_feedback).css({"color":"red","font-weight":"bold"});
                    ...
                    if (isFormValid) {
                        ...
                        const data = { 
                            ...
                            resume_introduction: resume_introduction,
                            resume_picme: resume_picme,
                        ...
                        document.querySelectorAll('.exp-item').forEach(item => {
                            data.resume_experiences.push({
                                ID: i+1,
                                在職時間: item.querySelector('.in-period').value,
                                公司: item.querySelector('.in-company').value,
                                職稱: item.querySelector('.in-title').value
                            });
                        });i++;
                        ...
                        document.querySelectorAll('.skill-item').forEach((item, index) => {
                            ...
                            data.resume_skills.push({
                                ID: j+1,
                                type: item.querySelector('.in-type').value,
                                skill: item.querySelector('.in-name').value,
                                trained: checked ? checked.value : 0
                            });
                        });j++;
...
alert(resume_name);
                        ...
                        $.ajax({
                            ...
                            success: function(response){
                                ...
                                $('#feedback2').html('<span style="color:green;">' + response.success2 + '</span>');
                                window.location.href = '/present/manage';
                            },
                            error: function(response){
                                // 验证失败时展示红色的错误提示
                                $('#feedback').html('<span style="color:red;">' + response.responseJSON.errors.resume_name[0] + '</span>');
                            }
                        });
                    }
                });
